...
public class TramiteWorkflowJdbcAdapter implements TramiteWorkflowPort {
    ...
    @Override
    public void derivarPlanTesis(Integer idPlanTesis, Long idUsuarioAccion, EstadoTramiteTesis estadoNuevo,
    ...
    @Override
    public List<Integer> findPlanTesisIdsByRevisor(RolRevisor rolRevisor) {
    ...
    public Integer registrarResolucion(Integer idPlanTesis, Long idUsuarioAccion,
        ...
        jdbcTemplate.update("""
            UPDATE tramites
               SET estado_actual = ?, rol_revisor_actual = ?, fecha_actualizacion = NOW()
             WHERE id_tramite = ?
            """, EstadoTramiteTesis.APROBADO_CON_RESOLUCION.name(),
                RolRevisor.SIN_REVISOR.name(), idTramite);
        registrarMovimiento(idTramite, idUsuarioAccion, "REGISTRAR_RESOLUCION",
                estadoAnterior, EstadoTramiteTesis.APROBADO_CON_RESOLUCION.name(),
