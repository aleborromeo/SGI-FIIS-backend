...
public class DashboardRepositoryImpl implements DashboardRepository {
    ...
    public DashboardAdmin obtenerDashboardAdmin(Integer idUsuario) {
        ...
        int totalProyectos = count("SELECT COUNT(*) FROM proyectos");
        int proyectosActivos = count("SELECT COUNT(*) FROM proyectos WHERE estado_proyecto = 'EN_EJECUCION'");
        int tramitesPendientes = count("SELECT COUNT(*) FROM tramites WHERE estado_actual NOT IN ('APROBADO','RECHAZADO','FINALIZADO')");
        int resolucionesEmitidas = count("SELECT COUNT(*) FROM resoluciones");
        ...
        // Alerta: convocatorias abiertas
        int convocatoriasAbiertas = count("SELECT COUNT(*) FROM convocatorias WHERE estado = 'ABIERTA'");
    ...
    public DashboardDirector obtenerDashboardDirector(Integer idUsuario) {
        ...
        int tramitesPendientesRevision = count(
                "SELECT COUNT(*) FROM tramites WHERE rol_revisor_actual = 'DIRECTOR_INVESTIGACION' AND estado_actual NOT IN ('APROBADO','RECHAZADO')");
    ...
    public DashboardCoordinador obtenerDashboardCoordinador(Integer idUsuario) {
        ...
        // Obtener el grupo del coordinador
        String sqlGrupo = """
        ...
        // Métricas del grupo (RF-91: solo de su grupo)
        int totalMiembros = count("SELECT COUNT(*) FROM membresias_grupo WHERE id_grupo = " + idGrupo);
        int miembrosActivos = count("SELECT COUNT(*) FROM membresias_grupo WHERE id_grupo = " + idGrupo + " AND es_activo = TRUE");
        int totalProyectosGrupo = count("SELECT COUNT(*) FROM proyectos WHERE id_grupo = " + idGrupo);
        int proyectosActivosGrupo = count("SELECT COUNT(*) FROM proyectos WHERE id_grupo = " + idGrupo + " AND estado_proyecto = 'EN_EJECUCION'");
        int tramitesPendientesGrupo = count("SELECT COUNT(*) FROM tramites WHERE id_grupo = " + idGrupo + " AND estado_actual NOT IN ('APROBADO','RECHAZADO')");
        int informesAvanceGrupo = count("SELECT COUNT(*) FROM informes_avance ia INNER JOIN proyectos p ON ia.id_proyecto = p.id_proyecto WHERE p.id_grupo = " + idGrupo);
        int planesTesisGrupo = count("SELECT COUNT(*) FROM planes_tesis WHERE id_grupo = " + idGrupo);

        int tramitesPostulados = count("SELECT COUNT(*) FROM tramites WHERE id_grupo = " + idGrupo + " AND estado_actual = 'POSTULADO'");
        int tramitesEnRevision = count("SELECT COUNT(*) FROM tramites WHERE id_grupo = " + idGrupo + " AND estado_actual = 'EN_REVISION'");
        int tramitesAprobados = count("SELECT COUNT(*) FROM tramites WHERE id_grupo = " + idGrupo + " AND estado_actual = 'APROBADO'");
        int tramitesObservados = count("SELECT COUNT(*) FROM tramites WHERE id_grupo = " + idGrupo + " AND estado_actual = 'OBSERVADO'");
    ...
    public DashboardDocente obtenerDashboardDocente(Integer idUsuario) {
        ...
        int proyectosComoResponsable = count("SELECT COUNT(*) FROM proyectos WHERE id_responsable = " + idUsuario);
        ...
        int tramitesPendientes = count(
                "SELECT COUNT(*) FROM tramites WHERE id_solicitante = " + idUsuario + " AND estado_actual NOT IN ('APROBADO','RECHAZADO')");
        int informesAvancePendientes = count(
                "SELECT COUNT(*) FROM informes_avance ia INNER JOIN proyectos p ON ia.id_proyecto = p.id_proyecto WHERE p.id_responsable = " + idUsuario + " AND ia.estado_informe = 'PENDIENTE'");
        int documentosCargados = count("SELECT COUNT(*) FROM documentos WHERE id_usuario_subio = " + idUsuario + " AND es_activo = TRUE");
        int resolucionesRecibidas = count(
                "SELECT COUNT(*) FROM resoluciones r INNER JOIN tramites t ON r.id_tramite = t.id_tramite WHERE t.id_solicitante = " + idUsuario);

        int proyectosPostulados = count("SELECT COUNT(*) FROM proyectos WHERE id_responsable = " + idUsuario + " AND estado_proyecto = 'POSTULADO'");
        int proyectosAprobados = count("SELECT COUNT(*) FROM proyectos WHERE id_responsable = " + idUsuario + " AND estado_proyecto = 'APROBADO'");
        int proyectosEnEjecucion = count("SELECT COUNT(*) FROM proyectos WHERE id_responsable = " + idUsuario + " AND estado_proyecto = 'EN_EJECUCION'");
        int proyectosFinalizados = count("SELECT COUNT(*) FROM proyectos WHERE id_responsable = " + idUsuario + " AND estado_proyecto = 'FINALIZADO'");
    ...
    public DashboardEvaluador obtenerDashboardEvaluador(Integer idUsuario) {
        ...
        int evaluacionesAsignadas = count("SELECT COUNT(*) FROM evaluaciones WHERE id_evaluador = " + idUsuario);
        int evaluacionesPendientes = count("SELECT COUNT(*) FROM evaluaciones WHERE id_evaluador = " + idUsuario + " AND fecha_evaluacion IS NULL");
        int evaluacionesCompletadas = count("SELECT COUNT(*) FROM evaluaciones WHERE id_evaluador = " + idUsuario + " AND fecha_evaluacion IS NOT NULL");
        int proyectosAsignados = count("SELECT COUNT(*) FROM evaluaciones WHERE id_evaluador = " + idUsuario + " AND id_proyecto IS NOT NULL");
        int planesTesisAsignados = count("SELECT COUNT(*) FROM evaluaciones WHERE id_evaluador = " + idUsuario + " AND id_plan_tesis IS NOT NULL");

        int evaluacionesAprobadas = count("SELECT COUNT(*) FROM evaluaciones WHERE id_evaluador = " + idUsuario + " AND resultado = 'APROBADO'");
        int evaluacionesRechazadas = count("SELECT COUNT(*) FROM evaluaciones WHERE id_evaluador = " + idUsuario + " AND resultado = 'RECHAZADO'");
        int evaluacionesConObservaciones = count("SELECT COUNT(*) FROM evaluaciones WHERE id_evaluador = " + idUsuario + " AND resultado = 'CON_OBSERVACIONES'");
    ...
    public DashboardDecano obtenerDashboardDecano(Integer idUsuario) {
        ...
        int totalProyectosFacultad = count("SELECT COUNT(*) FROM proyectos");
        int proyectosActivos = count("SELECT COUNT(*) FROM proyectos WHERE estado_proyecto = 'EN_EJECUCION'");
        int tramitesPendientesFirma = count("SELECT COUNT(*) FROM tramites WHERE rol_revisor_actual = 'DECANO' AND estado_actual NOT IN ('APROBADO','RECHAZADO')");
        int resolucionesEmitidas = count("SELECT COUNT(*) FROM resoluciones");
        int convocatoriasActivas = count("SELECT COUNT(*) FROM convocatorias WHERE estado = 'ABIERTA'");
        int totalGruposActivos = count("SELECT COUNT(*) FROM grupos_investigacion WHERE es_activo = TRUE");

        int tramitesEnEspera = tramitesPendientesFirma;

        // Trámites aprobados y rechazados en el mes actual
    ...
    public DashboardEstudiante obtenerDashboardEstudiante(Integer idUsuario) {
        ...
        // Estado del plan de tesis más reciente
