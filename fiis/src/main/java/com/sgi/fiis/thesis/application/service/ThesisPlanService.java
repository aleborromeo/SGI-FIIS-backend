...
public class ThesisPlanService implements ThesisPlanUseCase {
    ...
    public ThesisPlanResponse registrarPlan(RegisterThesisPlanCommand command) {
        ...
        Integer idTramite = tramiteWorkflow.crearTramitePlanTesis(guardado.getIdPlanTesis(), guardado.getIdEstudiante(),
                guardado.getIdGrupo());
    ...
    private Long extraerIdEstudianteDelContexto() {
        ...
        if (!hasRole(user, ROLE_ESTUDIANTE)) {
    ...
    public ThesisPlanResponse aprobarPorCoordinador(Integer idPlanTesis) {
        ...
        if (!grupoValidation.esCoordinadorDelGrupo(idUsuarioAccion, plan.getIdGrupo())) {
            throw new BusinessRuleViolationException("thesis.error.manage-wrong-group");
        }
    ...
    public ThesisPlanResponse observarPorCoordinador(Integer idPlanTesis, ObserveThesisPlanCommand command) {
        ...
        Long idUsuarioAccion = extraerIdUsuarioDelContexto();
        if (!grupoValidation.esCoordinadorDelGrupo(idUsuarioAccion, plan.getIdGrupo())) {
            throw new BusinessRuleViolationException("thesis.error.observe-wrong-group");
        }
    ...
    public ThesisPlanResponse rechazarPorCoordinador(Integer idPlanTesis, String motivo) {
        ...
        Long idUsuarioAccion = extraerIdUsuarioDelContexto();
        if (!grupoValidation.esCoordinadorDelGrupo(idUsuarioAccion, plan.getIdGrupo())) {
            throw new BusinessRuleViolationException("thesis.error.reject-wrong-group");
        }
    ...
    public ThesisPlanResponse observarPorDirector(Integer idPlanTesis, ObserveThesisPlanCommand command) {
        ...
        ThesisPlan guardado = planRepository.save(plan);
        Long idUsuarioAccion = extraerIdUsuarioDelContexto();
        // RN-07: Director observation returns to coordinator, not directly to student
        // (RF-50)
        tramiteWorkflow.derivarPlanTesis(idPlanTesis, idUsuarioAccion, ThesisProcedureStatus.OBSERVADO,
                ReviewerRole.COORDINADOR_GRUPO, "OBSERVAR_DIRECTOR", command.observacion(),
    ...
    public ThesisPlanResponse subsanarPlan(Integer idPlanTesis, RectifyThesisPlanCommand command) {
        ...
        if (command.idDocumentoActual() != null
                && !documentoValidation.existeDocumentoActivo(command.idDocumentoActual())) {
            throw new BusinessRuleViolationException("thesis.error.rectification-doc-inactive");
        }
        if (command.idDocumentoActual() == null
                && (command.resumenSubsanado() == null || command.resumenSubsanado().isBlank())) {
        ...
        tramiteWorkflow.derivarPlanTesis(idPlanTesis, idEstudiante, ThesisProcedureStatus.SUBSANADO,
                ReviewerRole.COORDINADOR_GRUPO, "SUBSANAR_PLAN_TESIS", command.comentarioSubsanacion(),
                command.idDocumentoActual());
        return toResponse(guardado);
    }
    ...
    public List<ThesisPlanResponse> listarPorEstudiante(Long idEstudiante) {
        ...
        if (esDecano) {
            return resultados.stream()
                    .filter(p -> p.estadoTramite() != null
                            && p.estadoTramite() == ThesisProcedureStatus.PENDIENTE_DECANATO)
                    .toList();
        }
    ...
    private boolean esRolDecano() {
        try {
            return hasRole(getAuthenticatedUser(), ROLE_DECANO);
    ...
    public List<ThesisPlanResponse> listarPorGrupo(Integer idGrupo) {
        ...
        if (hasRole(userDetails, ROLE_ESTUDIANTE)) {
            throw new BusinessRuleViolationException(
                    "Los estudiantes no tienen permisos para listar planes de tesis de un grupo");
        }
        if (hasRole(userDetails, ROLE_COORDINADOR_GRUPO)
                && !grupoValidation.esCoordinadorDelGrupo(userDetails.getId(), idGrupo)) {
            throw new BusinessRuleViolationException(
                    "No tiene permisos para ver planes de tesis de otro grupo de investigación");
        }
    ...
    private void validarRolCoordinador() {
        if (!hasRole(getAuthenticatedUser(), ROLE_COORDINADOR_GRUPO)) {
    ...
    private void validarRolDirector() {
        if (!hasRole(getAuthenticatedUser(), ROLE_DIRECTOR_INVESTIGACION)) {
            throw new BusinessRuleViolationException(
                    "Solo los directores de investigación pueden realizar esta acción");
        }
    }

    private void validarRolDecano() {
        if (!hasRole(getAuthenticatedUser(), ROLE_DECANO)) {
    ...
    private Long resolverIdEstudianteSegunRol(Long idEstudiante) {
        CustomUserDetails user = getAuthenticatedUser();
        return hasRole(user, ROLE_ESTUDIANTE) ? user.getId() : idEstudiante;
    }

    private void validarRevisorParaRol(ReviewerRole revisor) {
        CustomUserDetails userDetails = getAuthenticatedUser();
        boolean autorizado = switch (revisor) {
            case ESTUDIANTE -> hasRole(userDetails, ROLE_ESTUDIANTE);
            case COORDINADOR_GRUPO -> hasRole(userDetails, ROLE_COORDINADOR_GRUPO);
            case DIRECTOR_INVESTIGACION -> hasRole(userDetails, ROLE_DIRECTOR_INVESTIGACION);
            case DECANO -> hasRole(userDetails, ROLE_DECANO);
    ...
    private void validarAccesoPlan(ThesisPlan plan) {
        ...
        if (hasRole(userDetails, ROLE_DIRECTOR_INVESTIGACION)
                && tieneAccesoPorEstado(plan, ROLE_DIRECTOR_INVESTIGACION)) {
            return;
        }
    ...
    private void validarGrupoLineaYDocumento(Integer idGrupo, Integer idLinea, Integer idDocumento, Long idUsuario) {
        if (!grupoValidation.existeGrupoActivo(idGrupo))
            throw new BusinessRuleViolationException("thesis.error.group-inactive");
        if (!grupoValidation.existeLineaActiva(idLinea))
            throw new BusinessRuleViolationException("thesis.error.line-inactive");
        if (!grupoValidation.lineaPerteneceAlGrupo(idGrupo, idLinea))
            throw new BusinessRuleViolationException("thesis.error.line-not-belong-to-group");
        if (idDocumento != null) {
    ...
    private ThesisPlanResponse toResponse(ThesisPlan p, Integer idTramite) {
        try {
            ...
            String[] estudiante = resolveStudentName(p.getIdEstudiante(), p.getIdPlanTesis());
    ...
    private String[] resolveStudentName(Long idEstudiante, Integer planId) {
        try {
            var row = jdbcTemplate.queryForMap("SELECT nombres, apellidos FROM usuarios WHERE id_usuario = ?",
                    idEstudiante);
            return new String[] { (String) row.get("nombres"), (String) row.get("apellidos") };
        } catch (Exception e) {
            log.warn("Error resolving student name for plan {}: {}", planId, e.getMessage());
            return new String[] { null, null };
        }
    }

    private String[] resolveGroupInfo(Integer idGrupo, Integer planId) {
        try {
            var row = jdbcTemplate.queryForMap(
                    "SELECT nombre_grupo, codigo_grupo FROM grupos_investigacion WHERE id_grupo = ?", idGrupo);
            return new String[] { (String) row.get("nombre_grupo"), (String) row.get("codigo_grupo") };
        } catch (Exception e) {
            log.warn("Error resolving group info for plan {}: {}", planId, e.getMessage());
            return new String[] { null, null };
        }
    }

    private String resolveLineName(Integer idLinea, Integer planId) {
        try {
            var row = jdbcTemplate.queryForMap("SELECT nombre_linea FROM lineas_investigacion WHERE id_linea = ?",
                    idLinea);
            return (String) row.get("nombre_linea");
        } catch (Exception e) {
            log.warn("Error resolving line name for plan {}: {}", planId, e.getMessage());
            return null;
        }
    }

    private String resolveDocumentName(Integer idDocumento, Integer planId) {
        if (idDocumento == null)
            return null;
        try {
            var row = jdbcTemplate.queryForMap("SELECT nombre_original FROM documentos WHERE id_documento = ?",
                    idDocumento);
