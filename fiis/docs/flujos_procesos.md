...
sequenceDiagram
    ...
    RegisterUseCase->>UsuarioRepository: existsByCorreo(correo)?
    RegisterUseCase->>RegisterUseCase: Generar código de 6 dígitos
    RegisterUseCase->>PendingRegistrationService: register(correo, dto, code)
    Note over PendingRegistrationService: Expira en 5 minutos (ConcurrentHashMap)
    RegisterUseCase->>EmailSender: sendVerificationCode(correo, code)
    ...
    AuthController->>VerifyRegistrationUseCase: execute(correo, codigo)
    VerifyRegistrationUseCase->>PendingRegistrationService: get(correo)
    ...
    alt No existe o expiró
        ...
        VerifyRegistrationUseCase->>PendingRegistrationService: remove(correo)
        VerifyRegistrationUseCase->>TokenProvider: generateToken(correo, rol)
...
### Cambio de Contraseña (`POST /api/v1/auth/cambiar-password`)
...
### Reinicio de Contraseña (`PATCH /api/v1/usuarios/{id}/reiniciar-password`)
